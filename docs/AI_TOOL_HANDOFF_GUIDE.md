...
- **Always grep `migrations/` before picking a migration filename number.** Filenames collide silently — there is no auto-increment and the convention is `NNN_*.sql` in append-only order. Last assigned at time of writing is `005_media_discovery_module.sql`; next agent should use `006_*`.
